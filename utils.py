...
if __name__ == "__main__":
    ...
    for ex in examples:
        HN_values = test_unanimity_game_value(
            ex["n"], set(range(ex["n"])), edges=ex["edges"], value_type="hn"
        )
        assert torch.allclose(HN_values, torch.Tensor(ex["values"]), rtol=1e-3)
        shapley_values = test_unanimity_game_value(
            ex["n"], set(range(ex["n"])), value_type="shapley"
        )
    ...
    [
        print(f"{str(c) : >15}: {weight_HN[i]:.4f}   {weight_shapley[i]:.4f}")
        for i, c in enumerate(coalitions)
    ]
